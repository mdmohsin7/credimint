	...
	actions: {
		...
		async sendMsgCancelLoan({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.CredimintCredimint.tx.sendMsgCancelLoan({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgLiquidStake({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.CredimintCredimint.tx.sendMsgLiquidStake({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgCancelLoan({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.CredimintCredimint.tx.msgCancelLoan({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCancelLoan:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgLiquidStake({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.CredimintCredimint.tx.msgLiquidStake({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgLiquidStake:Init Could not initialize signing client. Wallet is required.')
